...
exports.Admin = async function (shiliobj, Gid) {
    ...
    //对密码进行md5加密
    shiliobj.Gmima = md5(shiliobj.Gmima);
...
//查询一个管理员---按属性来查询-----相当于登录
exports.Singlequery = async function (whereobj) {
    //whereobj对象为查询条件
    //console.log('==============================')
    if (!whereobj.boolean) {
        //进行md5加密后与数据库中的MD5加密后的数据进行判断是否一样
        whereobj.Gmima = md5(whereobj.Gmima);
    } else {
        delete whereobj.boolean;
    }
...
//console.log(shili.Gzhanhao===whereobj.Gzhanhao,shili.Gmima===whereobj.Gmima)
    //进行逻辑验证,判断是否有值,并且账号和密码严格相等
    if (shili && shili.Gzhanhao === whereobj.Gzhanhao && shili.Gmima === whereobj.Gmima) {
