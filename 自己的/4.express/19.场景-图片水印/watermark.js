...
async function shuiyin() {
    ...
    /* const Promises = await Promise.all([Jimp.read(lon), Jimp.read(lon1)]);

    //调整图片大小
    Promises[0].contain(100, 100);

    //lon加到lon1上
    Promises[1].composite(Promises[0], 100, 100, {
        mode: Jimp.BLEND_MULTIPLY,
        opacitySource: 0.5,
    });

    await Promises[1].write(newmi); */
};
